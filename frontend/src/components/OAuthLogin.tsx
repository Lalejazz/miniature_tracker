...
const OAuthLogin: React.FC<OAuthLoginProps> = ({ onSuccess, className = '' }) => {
  ...
  if (!config || (!config.google_enabled && !config.facebook_enabled)) {
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      {config.facebook_enabled && (
        <button
          ...
          className="w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm bg-blue-600 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
